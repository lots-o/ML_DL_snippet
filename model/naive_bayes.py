...
class BernoulliNB(object):
    ...
    def _extract_information(self, X, y):
        ...
        )  # The number of documents of class C=k containing word w_t for every class and for each word in vocab
...
class MultinomialNB(object):
    ...
    def _extract_information(self, X, y):
        ...
        n_k_w_t = defaultdict(
            lambda: defaultdict(int)
        )  # The relative frequency of w_t in documents of class k with respect to the total number of words in documents of that class
        d_k = defaultdict(list)  # The vector of document class C=k
        for doc_embedding, label in zip(X, y):
            N_k[label] += 1
            d_k[label].append(doc_embedding)
...
def main():
    ...
    logger.info(f"Accuracy : {(y_preds==y_test).sum()/len(y_preds)}")
